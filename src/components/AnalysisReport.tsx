...
import {
  ChevronDown, Download, Search, Phone, MessageCircle, Bell, FileText,
...
function Panel({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <section className={`glass rounded-2xl p-5 md:p-6 ${className}`}>{children}</section>;
}

function AccordionSection({
  n, title, sub, defaultOpen = false, children,
}: { n: number; title: string; sub?: string; defaultOpen?: boolean; children: ReactNode }) {
  ...
  return (
    <section className="glass rounded-2xl overflow-hidden animate-fade-up">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between gap-4 p-5 md:p-6 text-left hover:bg-foreground/[0.02] transition cursor-pointer"
      >
        <div className="min-w-0">
          <p className="text-[10px] uppercase tracking-[0.3em] text-gold/80">Section {n.toString().padStart(2, "0")}</p>
          <h2 className="font-display text-2xl md:text-3xl mt-1 truncate">{title}</h2>
          {sub && <p className="text-sm text-muted-foreground mt-1">{sub}</p>}
        </div>
        <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-gold/30 bg-gold/10 text-gold transition-transform duration-300 ${open ? "rotate-180" : ""}`}>
...
export function AnalysisReport({ lead }: { lead: Lead }) {
  ...
  return (
    <div className="space-y-5">
      {/* Section 1 — Lead Overview (open by default) */}
      <AccordionSection n={1} title="Lead Overview" sub={`${lead.name} · ${lead.property}`} defaultOpen>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <p className="text-xs uppercase tracking-[0.3em] text-gold/80">Call Intelligence Report</p>
          <button className="glass flex items-center gap-2 rounded-xl px-4 py-2 text-sm hover:border-gold/30 transition">
            <Download className="h-4 w-4 text-gold" /> Export
          </button>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-5">
          {[
            { icon: MapPin, label: "Preferred Project", value: lead.property },
            ...
            { icon: Clock, label: "Call Duration", value: lead.callDuration },
      ...
      <AccordionSection n={3} title="Lead Analysis" sub="Real Estate Qualification Framework · BANT integrated">
            ...
            <div>
              ...
              <div className="mt-3 flex gap-1">
                {["LOW", "MEDIUM", "HIGH"].map((l) => (
                  <div key={l} className="h-1.5 flex-1 rounded-full bg-gradient-to-r from-gold-soft to-gold" />
                ))}
              </div>
            </div>
      ...
      <AccordionSection n={4} title="Objection Intelligence">
          ...
          {objections.map((o, i) => (
            <Panel key={i}>
              <div className="flex items-start justify-between gap-3">
                ...
                <StatusPill status={o.status as ParamStatus | "handled" | "partial"} />
              </div>
            </Panel>
          ))}
        </div>
      ...
      <AccordionSection n={5} title="Risk Detection" sub={`${risks.length} risks identified`}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <Panel>
      ...
      <AccordionSection n={6} title="Compliance Check" sub={`${complianceScore} / 3 mandatory disclosures captured`}>
        <Panel>
      ...
      <AccordionSection n={7} title="Behavioral Call Score">
        <Panel>
          ...
          <div className="mt-5 border-t border-foreground/5 pt-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="font-display text-2xl text-gold">8.2 / 10</span>
            </div>
            <div className="h-2 bg-foreground/5 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-gold-soft to-gold" style={{ width: "82%" }} />
            </div>
          </div>
        </Panel>
      </AccordionSection>

      {/* Section 8 — Deal Intelligence */}
      <AccordionSection n={8} title="Deal Intelligence">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
          <Panel>
            ...
            <div className="h-1.5 bg-foreground/5 rounded-full overflow-hidden mt-2">
              <div className="h-full bg-gradient-to-r from-gold-soft to-gold" style={{ width: "74%" }} />
            </div>
          </Panel>
          <Panel>
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Deal Value Band</p>
            <div className="font-display text-3xl mt-1">₹95L – 1 Cr</div>
      ...
      <AccordionSection n={9} title="Next Best Action">
        <Panel>
      ...
      {/* Section 10 — Conversation Timeline */}
      <AccordionSection n={10} title="Conversation Timeline" sub="Visual flow of the entire call">
        <Panel>
      ...
      {/* Section 11 — Key Commitment Moments */}
      <AccordionSection n={11} title="Key Commitment Moments">
        <Panel>
      ...
      {/* Section 12 — Overall Sentiment */}
      <AccordionSection n={12} title="Overall Sentiment">
        <Panel>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-center">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Sentiment</p>
              <div className="font-display text-4xl text-[color:var(--success)] mt-1">Positive</div>
            </div>
      ...
      {/* Section 13 — AI Conclusion */}
      <AccordionSection n={13} title="AI Conclusion">
        <section className="glass-strong rounded-3xl p-8 border-gold-gradient relative overflow-hidden">
      ...
      {/* Section 14 — Detailed Transcript (largest, at bottom) */}
      <AccordionSection n={14} title="Detailed Call Transcript">
        <Panel>
